...
describe('doGenerate - Image Editing', () => {
  ...
  it('should throw error for unsupported model', async () => {
    const imageData = new Uint8Array([137, 80, 78, 71]);
    const dalleModel = provider.image('dall-e-3');
  ...
  it('should include response data with correct endpoint', async () => {
    ...
    const customModel = new OpenAIImageModel(
      ...
      {
        provider: 'test-provider',
        url: ({ path, modelId }: { path: string; modelId: string }) => `https://api.openai.com/v1${path}`,
        headers: () => ({}),
        _internal: {
          currentDate: () => testDate,
        },
  ...
  it('should pass headers for edit requests', async () => {
    ...
    await providerWithHeaders.image('dall-e-2').doGenerate({
      prompt: 'Edit this image',
      n: 1,
      size: '512x512',
      aspectRatio: undefined,
